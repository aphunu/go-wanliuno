...
func (s *simulation) launchNode(log bool) *Network {
	...
	ip[0] = 10
	addr := &net.UDPAddr{IP: ip, Port: 50303}
...
func (st *simTransport) sendPing(remote *Node, remoteAddr *net.UDPAddr, topics []Topic) []byte {
	...
	st.sendPacket(remote.ID, ingressPacket{
		...
		data: &ping{
			Version:    4,
			From:       rpcEndpoint{IP: st.senderAddr.IP, UDP: uint16(st.senderAddr.Port), TCP: 50303},
			To:         rpcEndpoint{IP: remoteAddr.IP, UDP: uint16(remoteAddr.Port), TCP: 50303},
