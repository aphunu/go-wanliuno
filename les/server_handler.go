...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Wanliuno peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light Wanliuno handshake failed", "err", err)
		return err
	}
	if p.server {
	...
	if !h.server.clientPool.connect(p, 0) {
		p.Log().Debug("Light Wanliuno peer registration failed", "err", errFullClientPool)
		return errFullClientPool
	}
	// Register the peer locally
	if err := h.server.peers.register(p); err != nil {
		h.server.clientPool.disconnect(p)
		p.Log().Error("Light Wanliuno peer registration failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light wanliuno response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Wanliuno message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Wanliuno message arrived", "code", msg.Code, "bytes", msg.Size)
