...
import (
	...
	"github.com/wanliuno/go-wanliuno/les/utils"
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local wanliuno node is running in archive mode, advertise ourselves we have
...
// participating in the Light Wanliuno sub-protocol.
