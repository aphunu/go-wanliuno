...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useWanliunoV = true // Default to use V = 27 or 28, the legacy Wanliuno format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useWanliunoV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Wanliuno PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useWanliunoV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Wanliuno ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Wanliuno Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useWanliunoV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
