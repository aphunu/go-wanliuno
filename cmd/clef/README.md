...
This setup allows a DApp to connect to a remote Wanliuno node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Wanliuno node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.wanliuno/keystore")
...
In this case, `wanliuno` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Geth todos
    - The signer should pass the `Origin` header as call-info to the UI. As of right now, the way that info about the request is put towanliunoer is a bit of a hack into the HTTP server. This could probably be greatly improved.
    - Relay: Geth should be started in `wanliuno --signer localhost:8550`.
...
The signer will generate a new private key, encrypt it according to [web3 keystore spec](https://github.com/wanliuno/wiki/wiki/Web3-Secret-Storage-Definition) and store it in the keystore directory.  
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/wanliuno/EIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [EIP-712](https://github.com/wanliuno/EIPs/blob/master/EIPS/eip-712.md) and returns the calculated signature.
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.wanliuno/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.wanliuno/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for wanliuno addresses.
