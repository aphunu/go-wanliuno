...
// Package utils contains internal helper functions for go-wanliuno commands.
package utils
...
import (
	...
	"github.com/wanliuno/go-wanliuno/common/fdlimit"
	...
	"github.com/wanliuno/go-wanliuno/graphql"
	"github.com/wanliuno/go-wanliuno/internal/ethapi"
	"github.com/wanliuno/go-wanliuno/internal/flags"
	"github.com/wanliuno/go-wanliuno/les"
	"github.com/wanliuno/go-wanliuno/log"
	"github.com/wanliuno/go-wanliuno/metrics"
	"github.com/wanliuno/go-wanliuno/metrics/exp"
	"github.com/wanliuno/go-wanliuno/metrics/influxdb"
	...
	whisper "github.com/wanliuno/go-wanliuno/whisper/whisperv6"
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `wanliuno account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Wanliuno client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Wanliuno service: %v", err)
		}
...
// RegisterEthStatsService configures the Wanliuno Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Wanliuno Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "wanliuno.", tagsMap)
		}
...
// new format.
//
// e.g. wanliuno account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// wanliuno --keystore /tmp/mykeystore --lightkdf account new
//
