...
import (
	...
	math2 "github.com/wanliuno/go-wanliuno/common/math"
...
// newAlethGenesisSpec converts a go-wanliuno genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-wanliuno and aleth
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-wanliuno/pull/11039,
...
// newParityChainSpec converts a go-wanliuno genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-wanliuno and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/wanliuno/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/wanliuno/EIPs/blob/master/EIPS/eip-607.md
	...
	// wanliuno has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Wanliuno.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Wanliuno.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyWanliunoGenesisSpec represents the genesis specification format used by the
// Python Wanliuno implementation.
type pyWanliunoGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyWanliunoGenesisSpec converts a go-wanliuno genesis block into a Parity specific
// chain specification format.
func newPyWanliunoGenesisSpec(network string, genesis *core.Genesis) (*pyWanliunoGenesisSpec, error) {
	// Only ethash is currently supported between go-wanliuno and pywanliuno
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyWanliunoGenesisSpec{
