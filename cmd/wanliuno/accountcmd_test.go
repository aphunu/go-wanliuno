...
func TestAccountListEmpty(t *testing.T) {
	wanliuno := runGeth(t, "account", "list")
	wanliuno.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	wanliuno := runGeth(t, "account", "list", "--datadir", datadir)
	defer wanliuno.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	wanliuno := runGeth(t, "account", "new", "--lightkdf")
	defer wanliuno.ExpectExit()
	wanliuno.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	wanliuno := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer wanliuno.ExpectExit()
	wanliuno.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	wanliuno := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	wanliuno := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer wanliuno.ExpectExit()
	wanliuno.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(wanliuno.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(wanliuno.StderrText(), m) {
...
// https://github.com/wanliuno/go-wanliuno/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	wanliuno.SetTemplateFunc("keypath", func(file string) string {
