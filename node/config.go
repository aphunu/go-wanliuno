...
import (
	...
	"github.com/wanliuno/go-wanliuno/accounts/external"
	"github.com/wanliuno/go-wanliuno/accounts/keystore"
	"github.com/wanliuno/go-wanliuno/accounts/scwallet"
	"github.com/wanliuno/go-wanliuno/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of wanliuno is "wanliuno". If no
...
func (c *Config) NodeName() string {
	...
	if name == "wanliuno" || name == "wanliuno-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "wanliuno" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by wanliuno 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'wanliuno' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-wanliuno-keystore")
		ephemeral = keydir
	}
