...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-wanliuno/build/test/createRandomStateTest\" wanliuno/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pywanliuno
	OUTPUT_PYTHON="$(docker run --rm wanliuno/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm wanliuno/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm wanliuno/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
