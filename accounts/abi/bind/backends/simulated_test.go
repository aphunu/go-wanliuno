...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != wanliuno.NotFound {
		t.Fatalf("err should be `wanliuno.NotFound` but received %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     wanliuno.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", wanliuno.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", wanliuno.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", wanliuno.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", wanliuno.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", wanliuno.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", wanliuno.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, wanliuno.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, wanliuno.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, wanliuno.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, wanliuno.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
