...
import (
	...
	"github.com/wanliuno/go-wanliuno/accounts/usbwallet/trezor"
...
// or whether the Wanliuno app was not started on it.
...
// Wanliuno address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.WanliunoAddress)
	if _, err := w.trezorExchange(&trezor.WanliunoGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.WanliunoTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.WanliunoTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
