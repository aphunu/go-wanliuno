...
// NewProtocolManager returns a new Wanliuno sub protocol manager. The Wanliuno sub protocol manages peers capable
// with the Wanliuno network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Wanliuno peer", "peer", id)

	// Unregister the peer from the downloader and Wanliuno peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Wanliuno protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Wanliuno peer connected", "name", p.Name())

	// Execute the Wanliuno handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Wanliuno handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("Wanliuno peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Wanliuno message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Wanliuno sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Wanliuno network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
